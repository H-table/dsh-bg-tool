 ...
 * The venv and models live under a `.bg-tools` environment root, resolved in
 * order: $DSH_BG_TOOL_ROOT override → $DSH_HOME/.bg-tools (or ~/.dsh/.bg-tools)
 * → the legacy workspace path (migration fallback) → $DSH_HOME/.bg-tools.
...
/** Legacy v1 location (workspace-bound); kept only as a migration fallback. */
const LEGACY_BG_ROOT = 'E:/ProjectCode/DeepSeekHarnessWorkbook/.bg-tools';

/**
 * Resolve the bg-tool environment root (venv + models + bg_server.py):
 *   1. $DSH_BG_TOOL_ROOT — explicit override;
 *   2. $DSH_HOME/.bg-tools (or ~/.dsh/.bg-tools) — the machine-local home;
 *   3. the legacy workspace path, when it still exists (migration fallback);
 *   4. $DSH_HOME/.bg-tools — the default for fresh machines.
 */
function resolveBgRoot() {
  const override = process.env.DSH_BG_TOOL_ROOT;
  if (override) return override;
  const dshHome = process.env.DSH_HOME || path.join(homedir(), '.dsh');
  const homeRoot = path.join(dshHome, '.bg-tools');
  if (existsSync(homeRoot)) return homeRoot;
  if (existsSync(LEGACY_BG_ROOT)) return LEGACY_BG_ROOT;
  return homeRoot;
}

const BG_ROOT = resolveBgRoot();
